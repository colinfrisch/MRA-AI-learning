    ...
    def test_main_simulation(self):
        ...
        if not user:
            ...
            db.user.update(where={'id': user.id}, data={
                           'current_chapter': {'disconnect': True}})
        ...
        if training:
            ...
            self.assertTrue(training.chapters and len(training.chapters)
                            > 0, "Training has no chapters")
            ...
            while chapter:
                ...
                self.assertEqual(user.current_chapter_id, chapter.id,
                ...
                chapter = self.user_manager.add_eval_to_current_training_chapter(
                    user, success)
                self.assertEqual(db.eval.count(
                    where={'user_id': user.id}), nb_chapter, "Invalid number of evals")

            # training is finished
            user = self.user_manager.get_user(user.id) or user
            self.assertIsNone(
                user.current_chapter, "User still has a current chapter after finishing training")
            self.assertEqual(self.user_manager.get_score_for_user(
