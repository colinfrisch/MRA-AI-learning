...
class UserManager:
    ...
    def add_eval_to_current_training_chapter(self, user, success: bool) -> Optional[Chapter]:
        if not user:
    ...
    def finish_current_training(self, user: User, training_id):
        ...
        user_with_trainings = self.db.user.find_first(where={'id': user.id}, include={
                                                      'finished_trainings': True})
