    ...
    def get_next_message(self):
        ...
        while not dialog_finished:
            ...
            if assistant_message.tool_calls:
                ...
                self.messages.append(
                    {"role": "assistant", "content": assistant_message.content, "display": False})
                print(
                    f"call Function name: {function_name} with arguments: {arguments}")

                if function_name == "get_training_list":
                    result = json.dumps(
                        self.training_manager.get_all_training_summaries())

                elif function_name == "get_all_training_summary_for_field":
                    result = json.dumps(
                        self.training_manager.get_all_training_summary_for_field(arguments["field"]))

                elif function_name == "create_training":
                    print(
                        "...Création d'un programme d'apprentissage avec : ", arguments["subject"])
